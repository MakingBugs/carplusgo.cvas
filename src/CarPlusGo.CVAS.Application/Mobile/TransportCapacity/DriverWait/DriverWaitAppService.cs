...
{
    public class DriverWaitAppService : IDriverWaitAppService
    {
        private readonly IMongoDBRepository _mongoDBRepository;
        public DriverWaitAppService(IMongoDBRepository orderRepository)
        {
            _mongoDBRepository = orderRepository;
        }

        #region 司机等待-页面1接口
        public Blanket4Dto Blanket(Blanket4ResultRequestDto input)
        {
            ...
            string[] OrderType1SuccessSpendTimeList = { "$updateTimeList.status15", "$updateTimeList.status10" };
            OrderType1SuccessSpendTimeDictionary.Add("$subtract", OrderType1SuccessSpendTimeList);

            Dictionary<string, object> OrderType1SuccessSpendTimeDictionary2 = new Dictionary<string, object>();
            object[] OrderType1SuccessSpendTimeList2 = { new BsonDocument(OrderType1SuccessSpendTimeDictionary), 0 };
            OrderType1SuccessSpendTimeDictionary2.Add("$max", OrderType1SuccessSpendTimeList2);

            var OrderType1SuccessSpendTimeGroup = new BsonDocument
            {
                { "_id", 1 },
                { "AvgSpendTime", new BsonDocument("$avg",new BsonDocument(OrderType1SuccessSpendTimeDictionary2)) },
            ...
            string[] OrderType20SuccessSpendTimeList = { "$updateTimeList.status15", "$appointmentTime" };
            ...
            string[] OrderType21SuccessSpendTimeList = { "$updateTimeList.status15", "$appointmentTime" };
            ...
            string[] OrderType22SuccessSpendTimeList = { "$updateTimeList.status15", "$appointmentTime" };
            ...
            string[] OrderType1CancelSpendTimeList = { "$calTime", "$updateTimeList.status10" };
            OrderType1CancelSpendTimeDictionary.Add("$subtract", OrderType1CancelSpendTimeList);

            Dictionary<string, object> OrderType1CancelSpendTimeDictionary2 = new Dictionary<string, object>();
            object[] OrderType1CancelSpendTimeList2 = { new BsonDocument(OrderType1CancelSpendTimeDictionary), 0 };
            OrderType1CancelSpendTimeDictionary2.Add("$max", OrderType1CancelSpendTimeList2);

            var OrderType1CancelSpendTimeGroup = new BsonDocument
            {
                { "_id", 1 },
                { "AvgSpendTime", new BsonDocument("$avg",new BsonDocument(OrderType1CancelSpendTimeDictionary2)) },
                { "OrderCount", new BsonDocument("$sum", 1) }
            };
            var OrderType1CancelSpendTimeAggregateFluent = _mongoDBRepository.Database.GetCollection<BsonDocument>("tab_order").Aggregate()
                .Match(builderFilter.And(builderFilter.Exists("updateTimeList.status10", true), builderFilter.Eq("orderStatus", 30), builderFilter.Eq("orderType", 1), builderFilter.Gte("updateTimeList.status1", input.From.ToLocalTime().Date), builderFilter.Lte("updateTimeList.status1", input.To.ToLocalTime().ToDayEnd())));
            ...
            var OrderType20CancelSpendTimeAggregateFluent = _mongoDBRepository.Database.GetCollection<BsonDocument>("tab_order").Aggregate()
                .Match(builderFilter.And(builderFilter.Exists("updateTimeList.status10", true), builderFilter.Eq("orderStatus", 30), builderFilter.Eq("orderType", 2), builderFilter.Eq("orderType2", 0), builderFilter.Gte("updateTimeList.status1", input.From.ToLocalTime().Date), builderFilter.Lte("updateTimeList.status1", input.To.ToLocalTime().ToDayEnd())));
            ...
            var OrderType21CancelSpendTimeAggregateFluent = _mongoDBRepository.Database.GetCollection<BsonDocument>("tab_order").Aggregate()
                .Match(builderFilter.And(builderFilter.Exists("updateTimeList.status10", true), builderFilter.Eq("orderStatus", 30), builderFilter.Eq("orderType", 2), builderFilter.Eq("orderType2", 1), builderFilter.Gte("updateTimeList.status1", input.From.ToLocalTime().Date), builderFilter.Lte("updateTimeList.status1", input.To.ToLocalTime().ToDayEnd())));
            ...
            var OrderType22CancelSpendTimeAggregateFluent = _mongoDBRepository.Database.GetCollection<BsonDocument>("tab_order").Aggregate()
                .Match(builderFilter.And(builderFilter.Exists("updateTimeList.status10", true), builderFilter.Eq("orderStatus", 30), builderFilter.Eq("orderType", 2), builderFilter.Eq("orderType2", 2), builderFilter.Gte("updateTimeList.status1", input.From.ToLocalTime().Date), builderFilter.Lte("updateTimeList.status1", input.To.ToLocalTime().ToDayEnd())));
        ...
        #region 司机等待-页面2接口
        public Period4Dto Period(Period4ResultRequestDto input)
        {
            var data = new Period4Dto();
        ...
        #region 司机等待-页面3接口
        public Dictionary<string, Period4Dto> ByTheHour(ByTheHour4ResultRequestDto input)
        {
            var datalist = new Dictionary<string, Period4Dto>();
        ...
        #region 抓取不同状态的数据
        protected Period4Dto GetPeriodDto(int type, Period4Dto data, int Period, int[] OrdersStatus, int? OrderType, int? OrderType2, DateTime? Day)
        {
        ...
        #region 动态周期
        protected IOrderedEnumerable<Period4DetailDto> GetOrderResult(int Period, int? OrderType, int? OrderType2, int? OrderStatus, DateTime? Day)
        {
        ...
        #region 以周为单位
        protected IOrderedEnumerable<Period4DetailDto> GetWeekPerformanceTrends(string dateFormat, DateTime beginTime, DateTime endTime, int? OrderType, int? OrderType2, int? OrderStatus)
        {
            var list = new List<Period4DetailDto>();
        ...
        #region 动态创建Group对象
        protected IOrderedEnumerable<Period4DetailDto> OrderGroup(string dateFormat, DateTime beginTime, DateTime endTime, int? OrderType, int? OrderType2, int? OrderStatus)
        {
            ...
            if(OrderStatus == null)
            {
                if (OrderType==1)
                {
                    SubtractBegin = "$updateTimeList.status15";
                    SubtractEnd= "$updateTimeList.status10";
                }
                else
                {
                    SubtractBegin = "$updateTimeList.status15";
        ...
        protected IAggregateFluent<BsonDocument> GetOrderAggregateFluent(string TimeKey, DateTime beginTime, DateTime endTime, int? OrderType, int? OrderType2, int? OrderStatus)
        {
            ...
            if (OrderStatus != null)
            {
                ...
                filterDefinitions.Add(builderFilter.Exists("updateTimeList.status10", true));
            }
            else
            {
