...
namespace CarPlusGo.CVAS.Roles
{
    ...
    public class RoleAppService : AsyncCrudAppService<Role, RoleDto, int, PagedRoleResultRequestDto, CreateRoleDto, RoleDto>, IRoleAppService
    {
        ...
        public override async Task<RoleDto> Get(EntityDto<int> input)
        {
            ...
            var roleOutput = MapToEntityDto(role);

            roleOutput.Permissions = (await _roleManager.GetGrantedPermissionsAsync(role)).Select(p => p.Name).ToList();
            return roleOutput;
        }
        ...
        public override async Task<PagedResultDto<RoleDto>> GetAll(PagedRoleResultRequestDto input)
        {
            ...
            var roleDtos = entities.Select(MapToEntityDto).ToList();

            foreach (var role in roleDtos)
            {
                role.Permissions = (await _roleManager.GetGrantedPermissionsAsync(role.Id)).Select(p => p.Name).ToList();
            }
